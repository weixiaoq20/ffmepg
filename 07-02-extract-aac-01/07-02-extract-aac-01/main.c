                ...
                const int channels)
{
   ...
   p_adts_header[0] = 0xff;         //syncword:0xfff                          高8bits
      ...
      p_adts_header[3] |= ((adstLen & 0x1800) >> 11);           //frame length：value   高2bits


      p_adts_header[4] = (uint8_t)((adstLen & 0x7f8) >> 3);     //frame length:value    中间8bits
      p_adts_header[5] = (uint8_t)((adstLen & 0x7) << 5);       //frame length:value    低3bits
...
int main(int argc,char * argv[]){
    ...
    in_filename=argv[1];//輸入文件
    aac_filename=argv[2];//輸出文件
...
//    }
    ...
    //if((ret=avformat_find_stream_info(&fmt_ctx,NULL))<0)  这里有个天坑的 会报错
...
//                        fmt_ctx->streams[audio_index]->codecpar->channels);
            ...
            adts_header(adts_headr_buf, pkt.size,
                        fmt_ctx->streams[audio_index]->codecpar->profile,
                        fmt_ctx->streams[audio_index]->codecpar->sample_rate,
                        fmt_ctx->streams[audio_index]->codecpar->channels);

            fwrite(adts_headr_buf,1,7,aac_fd);//写adts_header,ts流不适用，ts流分离出来的packet带了adts header
            len=fwrite(pkt.data,1,pkt.size,aac_fd);//写adts_data
